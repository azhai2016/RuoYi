...
public class ProductionCategoryServiceImpl implements IProductionCategoryService {
    ...
    public List<ProductionCategory> selectCategoryList(ProductionCategory productionCategory) {
    ...
    @Override
    @DataScope(categoryAlias = "d")
    public List<Ztree> selectCategoryTreeExcludeChild(ProductionCategory productionCategory) {
        Long excludeId = productionCategory.getExcludeId();
        List<ProductionCategory> prodCategoryList = productionCategoryMapper.selectCategoryList(productionCategory);
     ...
     * @return 树结构列表
     */
    public List<Ztree> initZtree(List<ProductionCategory> categoryList) {
     ...
     * @return 树结构列表
     */
    public List<Ztree> initZtree(List<ProductionCategory> pcList, List<String> pdcategoryList) {

        List<Ztree> ztrees = new ArrayList<Ztree>();
        boolean isCheck = StringUtils.isNotNull(pdcategoryList);
        for (ProductionCategory pc : pcList) {
     ...
     */
    @Override
    public int insertCategory(ProductionCategory productionCategory) {
        ProductionCategory info = productionCategoryMapper.selectCategoryById(productionCategory.getParentId());
    ...
    @Override
    @Transactional
    public int updateCategory(ProductionCategory productionCategory) {
        ProductionCategory newParentCategroy = productionCategoryMapper
                .selectCategoryById(productionCategory.getParentId());
        ProductionCategory oldDept = selectCategoryById(productionCategory.getCategoryId());
     ...
     */
    private void updateParentDeptStatusNormal(ProductionCategory productionCategory) {
    ...
    public void updateDeptChildren(Long deptId, String newAncestors, String oldAncestors) {
        List<ProductionCategory> children = productionCategoryMapper.selectChildrenCategoryById(deptId);
        for (ProductionCategory child : children) {
     ...
     */
    @Override
    public ProductionCategory selectCategoryById(Long categoryId) {
    ...
    public boolean checkCategoryNameUnique(ProductionCategory productionCategory) {
        ...
        ProductionCategory info = productionCategoryMapper.checkCategoryNameUnique(
    ...
    public void checkCategoryDataScope(Long categoryId) {
        if (!SysUser.isAdmin(ShiroUtils.getUserId())) {
            ProductionCategory dept = new ProductionCategory();
            dept.setCategoryId(categoryId);
            List<ProductionCategory> depts = SpringUtils.getAopProxy(this).selectCategoryList(dept);
